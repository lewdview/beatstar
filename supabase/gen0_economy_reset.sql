-- ============================================================================
-- PIM : th3v4ult — PUBLIC GEN 0 RESET BOUNDARY MIGRATION & SCRIPT
-- ============================================================================
-- Objective: Start Reddit launch with a completely clean, unpolluted economy
-- while preserving all 365 releases, artwork, card definitions, pack drop rules,
-- admin configs, and account/wallet merge infrastructure.
-- ============================================================================
...
-- Reset all earned/rolled card instances so nobody owns cards at launch
...
-- Purges development and QA high scores, combos, accuracy runs, and milestone claims
...
-- Reset promo code redemption logs while keeping the bonus codes themselves
TRUNCATE TABLE public.bonus_code_redemptions CASCADE;

-- 4. ARCHIVE CORE CREATOR / QA PROFILES
-- Mark creator & internal test profiles as Alpha to permanently isolate them
...
-- 6. ENSURE PUBLIC LEADERBOARD VIEW EXCLUDES ARCHIVED / QA ACCOUNTS
...
-- 7. VERIFY INTEGRITY OF IMMUTABLE ASSETS
DO $$
DECLARE
  v_release_count INT := 0;
  v_code_count INT := 0;
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'releases') THEN
    SELECT COUNT(*) INTO v_release_count FROM public.releases;
  END IF;

  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'bonus_codes') THEN
    SELECT COUNT(*) INTO v_code_count FROM public.bonus_codes;
  END IF;

  RAISE NOTICE '>>> PUBLIC GEN 0 RESET APPLIED SUCCESSFULLY <<<';
  RAISE NOTICE 'Preserved Releases: % | Preserved Bonus Codes: %', v_release_count, v_code_count;
END $$;
